...
const rows = [
  createData("Team 1", 1, "壓克力", 3, "雷切二"),
  createData("Team 2", 2, "密集板", 5, "無"),
];
...
export default function LaserCutter() {
  ...
  const [laserNo, setLaserNo] = useState("");
  const [open, setOpen] = useState(false); // 新增雷切機
  const handleOpen = () => setOpen(true); // 新增雷切機
  const handleClose = () => setOpen(false); // 新增雷切機
  const handleConfirm = () => {
    setOpen(false);
    // if (laserNo == '') setOpen(true);
    // else setOpen(false);
  };
  const modalStyle = {
    position: "absolute",
